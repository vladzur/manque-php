...
class Controller {
	...
	function show($view = null, $data = array(), $layout = "default") {
		...
		$data = (array) $data;
		$file_layout = WWW_ROOT . "/views/layout/" . $layout . ".tpl";
		$file_view = WWW_ROOT . "/views/$view_dir/$view.tpl";
		ob_start();
	...
	function error($error = 404, $layout = "default") {
		$file_layout = WWW_ROOT . "/views/layout/" . $layout . ".tpl";
		$file_view = WWW_ROOT . "/views/error/$error.tpl";
		ob_start();
		$html = new Html();
		$element = new Element();
		include $file_view;
	...
	function _loadModels($models = array()) {
		if (!empty($models)) {
			foreach ($models as $model) {
				$model_file = WWW_ROOT . "/models/" . strtolower($model) . ".php";
